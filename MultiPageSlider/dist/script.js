...
window.addEventListener('DOMContentLoaded', function () {
  ...
  new _modules_accordion__WEBPACK_IMPORTED_MODULE_4__["default"]('.plus__content', '.msg').init();
});
...
/*#__PURE__*/
function () {
  function Accordion(triggersSelector, message) {
    ...
    value: function btnTrigger() {
      this.btns.forEach(function (btn) {
        ...
        btn.addEventListener('click', function () {
          ...
          sibling.style.marginTop = '25px'; // sibling.classList.toggle('plus__content');
          ...
          console.log(card); // minus.appendChild(card)
...
function () {
  ...
  _createClass(Difference, [{
    key: "bindTrigers",
    value: function bindTrigers(cardOfice, cardNumb, cardItem) {
      cardOfice.querySelector('.plus').addEventListener('click', function () {
    ...
    key: "hideItems",
    value: function hideItems(cards) {
...
function () {
  ...
  _createClass(PlayVideo, [{
    ...
    value: function bindTrigers() {
      ...
      // подвязка тригеров к определеннім действиям
      this.btns.forEach(function (btn, num) {
        try {
        ...
        btn.addEventListener('click', function () {
          if (!btn.closest('.module__video-item') || btn.closest('.module__video-item').getAttribute('data-disabled') !== "true") {
            _this.activeBtn = btn; // btn - та Noda  тот елемент в которую кликнул пользователь
            ...
            if (document.querySelector('iframe#frame')) {
              ...
              if (_this.path !== btn.getAttribute('data-url')) {
                // проверяем тыкнул ли пользователь в кнопку
                _this.path = btn.getAttribute('data-url'); // this.path создаем екземпляр класа

                _this.player.loadVideoById({
                  //загружаем видео по определенному идентификатору
                  videoId: _this.path
                });
              }
            } else {
    ...
    value: function bindCloseBtn() {
      ...
      this.close.addEventListener('click', function () {
        ...
        _this2.player.stopVideo(); // this.player.clearVideo();
    ...
    value: function onPlayerStateChange(state) {
      try {
        var blockedElem = this.activeBtn.closest('.module__video-item').nextElementSibling; //получаем елемент которій заблокирован
        // Итого:
        ...
        var playBtn = this.activeBtn.querySelector('svg').cloneNode(true); // cloneNode - (создание клона) копирует обертку(например svg), а якщо cloneNode(true) - то копіює + ще вкладеність
    ...
    key: "init",
    value: function init() {
      // создаем новый метод. Назовем init()
      if (this.btns.length > 0) {
        // если мы ничего не передадим this.btns = document.querySelectorAll(trigger); то получим пустой массив []
...
function (_Slider) {
  ...
  _createClass(MainSlider, [{
    ...
    value: function bindTrigers() {
      ...
      this.btns.forEach(function (btn) {
        ...
        btn.parentNode.previousElementSibling.addEventListener('click', function (e) {
          //по клику на лого переключает слайд
      ...
      document.querySelectorAll('.prevmodule').forEach(function (arrow) {
      ...
      document.querySelectorAll('.nextmodule').forEach(function (arrow) {
        arrow.addEventListener('click', function (e) {
          e.stopPropagation();
          e.preventDefault();
    ...
    value: function render() {
      // єтот метод переключает слайд
      if (this.container) {
        // ЕСЛИ єто правда == true
        try {
          // try catch дает  возможность не поломать скрипт
          // в try  мы обявляем елемент this.hanson а потом используем ее в showSlides()
  ...
  _createClass(MiniSlider, [{
    ...
    value: function decorizeSlides() {
      ...
      if (!this.slides[0].closest('button')) {
        // closest - возвращает ближайшего родителя либо же себя если подходит по селектиору в данном случае 'button'
    ...
    value: function init() {
      ...
      try {
        ...
        if (this.autoplay) {
          // єто значит если этот параметр нм=ам передан
...
var Slider = // в класс мы передаем аргументы page, btns
...
function Slider() {
  ...
  ; // .children - мы получаем всех детей page/container
